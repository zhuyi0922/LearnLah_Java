...
public class RecordDetailServiceImpl implements RecordDetailService {
    ...
    public List<String> getReviewWordList() {
        ...
        // 2. 前7天到前3天背过2次的单词,
        List<String> wordList2 = recordDetailRepository.findByLastAttemptTimeBetweenAndStatus(sevenDaysAgo, threeDaysAgo, StatusEnum.Success_second)
                                                        .stream()
                                                        .map(x -> x.getWord().getWordId()).toList();

        // 3. 前15天到前7天背过3次的单词,
        List<String> wordList3 = recordDetailRepository.findByLastAttemptTimeBetweenAndStatus(fifteenDaysAgo, sevenDaysAgo, StatusEnum.Success_third)
                                                        .stream()
                                                        .map(x -> x.getWord().getWordId()).toList();
